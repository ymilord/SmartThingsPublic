...
preferences {
	section("Use this switch for the deck led strip..") {
		input "Deck", "capability.switch", title: "Deck LED Strip?"
	}
    ...
    section("Spark Core ID..") {
		input "SparkID", "parameters", title: "Spark Core Device ID?"
	}
...
def turnRelayOnDeck(group) {
    if (group == "") {
	 ...
	 def errorClosure = { response ->       
                log.debug "Spark Core Offline, $response"
    }
     def successClosure = { response ->       
                log.debug "Request was successful, $response"                                                                          
               }                                            
               def postBody = "access_token=${AccessToken}¶ms=r1%2CLOW"
               httpPost("https://api.spark.io/v1/devices/${SparkID}/ledstrip", postBody, successClosure)
}
...
def turnRelayOffDeck(group) {
    ...
    def successClosure = { response ->       
               ...
               def postBody = "access_token=${AccessToken}¶ms=r1%2CHIGH"
               httpPost("https://api.spark.io/v1/devices/${SparkID}/ledstrip", postBody, successClosure)
}
def turnRelayOnSprinkler(group) {
    if (group == "") {
     ...
     def successClosure = { response ->       
               ...
               def postBody = "access_token=${AccessToken}¶ms=r3%2CLOW"
               httpPost("https://api.spark.io/v1/devices/${SparkID}/ledstrip", postBody, successClosure)
}
def turnRelayOffSprinkler(group) {
    ...
    def successClosure = { response ->       
               ...
               def postBody = "access_token=${AccessToken}¶ms=r3%2CHIGH"
               httpPost("https://api.spark.io/v1/devices/${SparkID}/ledstrip", postBody, successClosure)
}